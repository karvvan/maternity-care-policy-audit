# -*- coding: utf-8 -*-
"""
시연 영상용 모션그래픽 소스 생성 (나레이션 없음 — 대표자가 직접 녹음해 얹는 용도)

보고서 핵심 내용을 파트별 애니메이션 클립(mp4, 1920x1080/30fps)으로 렌더링한다.
디자인은 PPT와 같은 다크 '오로라' 테마 (design_assets 배경 재사용).

사용법:
  python 영상소스_생성.py            # 전체 렌더
  python 영상소스_생성.py 3-1 5-3    # 지정 클립만 렌더
...
from matplotlib.patches import FancyBboxPatch, Circle, Polygon as MplPolygon, FancyArrowPatch, Wedge, Rectangle
...
FPS = 30
TXT, MUT = '#ECF1F8', '#9FB0C3'
ACC, ACC2, BLU, TEA, DANGER = '#FF7A59', '#FFB86B', '#8AB4FF', '#6EE7D2', '#FF6B6B'
GLASS = (1, 1, 1, 0.07)
BG_IMG = np.asarray(Image.open(ASSETS['bg_main']).convert('RGB')) / 255.0
...
def glass_box(ax, x, y, w, h, alpha=1.0, fc=GLASS, ec=(1, 1, 1, 0.20), lw=1.2, z=3):
    p = FancyBboxPatch((x, y), w, h, boxstyle='round,pad=0.6,rounding_size=1.6',
                       facecolor=(fc[0], fc[1], fc[2], fc[3] * alpha),
                       edgecolor=(ec[0], ec[1], ec[2], ec[3] * alpha), linewidth=lw, zorder=z)
    ax.add_patch(p)
    return p
...
def render(cid):
    ...
    def update(i):
        for a in list(ax.patches) + list(ax.texts) + list(ax.lines) + list(ax.collections):
            a.remove()
        fn(ax, i / (frames - 1), i / FPS, artists_state)
...
def c11(ax, t, sec, st):
    ...
    for name, desc, x, color, t0 in nodes:
        ...
        if a <= 0:
            continue
        y = 48 + (1 - a) * 6
        glass_box(ax, x, y, 40, 22, alpha=a)
        ax.add_patch(Rectangle((x, y + 20.5), 40 * a, 1.5, facecolor=color, edgecolor='none',
                               alpha=a, zorder=4))
...
def c12(ax, t, sec, st):
    ...
    for start, span, color, label in [(90, -72 * a1, BLU, '정량 20'),
                                      (90 - 72 * a1, -288 * a1, '#4A6EA8', '정성 80')]:
        if abs(span) > 1:
    ...
    n0 = int(0 + (1 - ease(seg(t, 0.45, 0.8))) * 100) if t > 0.45 else 100
    ...
    if a2 > 0:
        glass_box(ax, 104, 38, 74, 30, alpha=a2)
...
def c13(ax, t, sec, st):
    ...
    data = [('A', 32, ACC, '두 지표 모두 취약'), ('B', 21, ACC2, '하나만 취약'),
    ...
    for i, (g, n, color, desc) in enumerate(data):
        ...
        if a > 0:
            ax.add_patch(FancyBboxPatch((x, 52), w * a, 16, boxstyle='round,pad=0.4,rounding_size=1.2',
                                        facecolor=color, edgecolor='none', alpha=0.88, zorder=4))
            ax.text(x + w * a / 2, 60, f'{g} · {int(n * a)}곳', fontsize=21, fontweight='bold',
                    color='#101B2E', ha='center', va='center', zorder=5)
...
def c21(ax, t, sec, st):
    ...
    for i, (num, desc, t0) in enumerate(cards):
        ...
        x = 14 + (i % 2) * 86
        y = 52 - (i // 2) * 26 + (1 - a) * 4
        glass_box(ax, x, y, 78, 20, alpha=a)
        ax.text(x + 6, y + 10, num, fontsize=30, fontweight='bold', color=ACC2, va='center',
                alpha=a, zorder=5)
...
def c22(ax, t, sec, st):
    ...
    glass_box(ax, 24, 24, 52, 54, alpha=1)
    glass_box(ax, 122, 24, 52, 54, alpha=1)
    ...
    for (p, d, ph) in zip(st['pts'], st['dst'], st['ph']):
        ...
        ax.add_patch(Circle((cx, cy), 0.9, facecolor=ACC if u < 1 else TEA, edgecolor='none',
                            alpha=0.85, zorder=6))
    ax.add_patch(Circle((148, 51), 3.2, facecolor=TEA, edgecolor='white', lw=1.5, alpha=0.9, zorder=7))
...
def c31(ax, t, sec, st):
    ...
    ax.add_patch(Wedge((cx, cy), r, 90, 90 + 360, width=7.5, facecolor=(1, 1, 1, 0.10),
...
def c32(ax, t, sec, st):
    ...
    bars = [('이동시간 2변수', 1.0, BLU, 0.1), ('+ 전문의 수', 1.0, MUT, 0.38), ('+ 관내이용률', 1.0, MUT, 0.55)]
    for i, (name, h, color, t0) in enumerate(bars):
        a = ease(seg(t, t0, t0 + 0.18))
        if a <= 0:
            continue
        x = 30 + i * 46
        ...
        ax.text(x + 15, 30 + bh + 3.5, 'AUC 1.000' if a > 0.9 else '', fontsize=15,
                fontweight='bold', color=TXT, ha='center', zorder=5)
        ax.text(x + 15, 24.5, name, fontsize=14, color=MUT, ha='center', alpha=a, zorder=5)
    a2 = ease(seg(t, 0.75, 0.92))
    if a2 > 0:
        ax.text(96, 88, '개선 폭 0 — 공급 정보를 쥐여 줘도 판정을 더 잘 맞히지 못한다',
                fontsize=18, fontweight='bold', color=ACC2, ha='center', alpha=a2, zorder=5)
...
def c33(ax, t, sec, st):
    ...
    for i, (name, desc, t0) in enumerate(spots):
        ...
        glass_box(ax, x, y - 6, 27, 22, alpha=a)
...
def c34(ax, t, sec, st):
    ...
    ax.add_patch(Circle((54, 50), 26, facecolor=(1, 1, 1, 0.06), edgecolor=(1, 1, 1, 0.25),
                        lw=1.5, zorder=3))
...
def c41(ax, t, sec, st):
    ...
    if a2 > 0:
        sc = 0.8 + 0.2 * a2
        glass_box(ax, 138, 44, 42, 22, alpha=a2, fc=(1, 0.48, 0.35, 0.16), ec=(1, 0.6, 0.48, 0.4))
        ax.text(159, 57.5, f'오즈 {4.82 * a2:.2f}배', fontsize=26 * sc, fontweight='bold',
                color=ACC2, ha='center', zorder=6)
...
def c42(ax, t, sec, st):
    ...
    glass_box(ax, x0, y0, w, h, alpha=1)
    fill = ease(seg(t, 0.1, 0.5)) * 0.163
    ax.add_patch(FancyBboxPatch((x0, y0), w * fill / 1.0, h, boxstyle='round,pad=0.4,rounding_size=1.2',
                                facecolor=TEA, edgecolor='none', alpha=0.85, zorder=4))
    ax.text(x0 + w * 0.163 / 2, y0 + h / 2, f'{fill * 100:.0f}%', fontsize=19, fontweight='bold',
            color='#101B2E', ha='center', va='center', zorder=6)
    a1 = seg(t, 0.5, 0.62)
    if a1 > 0:
        qn = int(1 + a1 * 6)
        for i in range(qn):
            qx = x0 + w * 0.25 + i * w * 0.115
            ax.text(qx, y0 + h / 2, '?', fontsize=24, fontweight='bold', color=MUT,
...
def c51(ax, t, sec, st):
    ...
    for k in range(n_show):
        ...
        ax.add_patch(Circle((px, py), 1.15 + (1 - pop) * 1.2, facecolor=BLU if supported else ACC,
                            edgecolor='white', lw=0.5, alpha=0.9 * pop + 0.1, zorder=6))
    if t > 0.66:
        a = ease(seg(t, 0.66, 0.78))
        for nm, xs, yb, va in [('울릉 S1', 1, 53, 'top'), ('양평 B1', 17, 1, 'bottom'), ('진천 S53·B2', 53, 2, 'bottom')]:
            px = x0 + 4 + (xs - 1) / 52 * (w - 8)
            py = y0 + h - 4 - (yb - 1) / 52 * (h - 8)
            ax.add_patch(Circle((px, py), 2.1, facecolor='none', edgecolor=ACC2, lw=2, alpha=a, zorder=7))
            ax.text(px + 2.8, py, nm, fontsize=12.5, fontweight='bold', color=ACC2, va='center',
                    alpha=a, zorder=7)
    a2 = ease(seg(t, 0.82, 0.95))
    if a2 > 0:
...
def c52(ax, t, sec, st):
    ...
    rules = [('관내 수행 ≥50건', '분만 운영지원', '1곳', TEA, 0.06),
             ('인구·수요 충족 ∧ 수행<50', '분만 설치 검토', '7곳', ACC, 0.24),
             ('인구 미달 ∧ 병원급 ≥1', '외래 산부인과', '13곳', ACC2, 0.42),
    ...
    for i, (cond, out, n, color, t0) in enumerate(rules):
        ...
        if a <= 0:
            continue
        y = 74 - i * 15
        glass_box(ax, 16, y - 5, 66, 11, alpha=a)
        ...
        glass_box(ax, 102, y - 5, 52, 11, alpha=a, fc=(1, 1, 1, 0.10))
...
def c53(ax, t, sec, st):
    ...
    for i in range(7):
        ...
        ax.add_patch(Circle((x, 56), 5.5, facecolor=color if filled else 'none',
                            edgecolor=color, lw=2.5, alpha=a * (0.9 if filled else 0.85), zorder=5))
        ax.text(x, 56, '지원' if filled else '공백', fontsize=12.5, fontweight='bold',
                color='#101B2E' if filled else DANGER, ha='center', va='center', alpha=a, zorder=6)
...
def c54(ax, t, sec, st):
    ...
    # 지도 좌표 변환: lon 125.7~130 → x 62~152, lat 33.8~38.7 → y 4~96
    def tr(lon, lat):
    ...
    ax.add_collection(PatchCollection(patches, facecolor=(1, 1, 1, 0.05 * base_a),
                                      edgecolor=(1, 1, 1, 0.10 * base_a), linewidths=0.4, zorder=3))
    ...
    for _, polys in others:
        ax.add_collection(PatchCollection(polys, facecolor=(1, 0.68, 0.58, 0.35 * a_ab),
                                          edgecolor='none', zorder=4))
    for _, polys in blues:
        ax.add_collection(PatchCollection(polys, facecolor=(0.42, 0.62, 0.95, 0.6 * a_ab),
                                          edgecolor='none', zorder=4))
    ...
    for i, (row, polys) in enumerate(sorted(reds, key=lambda r: r[0]['순위S'])):
        ...
        if aa <= 0:
            continue
        ax.add_collection(PatchCollection(polys, facecolor=(1, 0.32, 0.25, min(1, 0.75 + pulse * 0.2) * aa),
                                          edgecolor='white', linewidths=0.6, zorder=5))
        arr = np.array(max(rings(row['geom']), key=len))
        cx, cy = tr(arr[:, 0].mean(), arr[:, 1].mean())
        ax.text(cx + 4 if row['시군'] != '신안군' else cx - 4, cy + 2.5,
                f"{row['시군']} S{row['순위S']}위", fontsize=12.5, fontweight='bold', color='#FFB3A6',
                alpha=aa, zorder=7, ha='left' if row['시군'] != '신안군' else 'right')
    a2 = ease(seg(t, 0.72, 0.88))
    if a2 > 0:
        glass_box(ax, 8, 24, 44, 34, alpha=a2)
        ax.text(30, 51.5, '심도 상위 6곳 전부 공백', fontsize=16.5, fontweight='bold', color='#FF9F92',
...
def c61(ax, t, sec, st):
    ...
    for i, (no, head, tail) in enumerate(items):
        ...
        y = 76 - i * 13.5
        x_off = (1 - a) * 8
        glass_box(ax, 20 + x_off, y - 5, 152, 10.5, alpha=a)
        ax.add_patch(Circle((28 + x_off, y + 0.3), 3.2, facecolor=ACC, edgecolor='none', alpha=a, zorder=5))
        ax.text(28 + x_off, y + 0.3, no, fontsize=15, fontweight='bold', color='#101B2E',
...
def c62(ax, t, sec, st):
    ...
    if a4 > 0:
        ax.text(96, 31, '우리는 정부 자신의 기준으로 그 축을 하나 추가했다.', fontsize=21,
                fontweight='bold', color=ACC2, ha='center', alpha=a4, zorder=5)
