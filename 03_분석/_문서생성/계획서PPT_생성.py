...
card(s, 0.55, 5.35, 12.35, 1.5, '왜 지금, 왜 데이터인가', [
    ...
    '지원은 시·군 1곳 → 시·도 원칙 1개소 → 복지부 심사를 거친다. 15년 누적 57개소, 2025년 분만 신규는 전국 1개소',
    '나눠 줄 것이 적을수록 순서는 오히려 더 중요해진다 — 한 해에 한 곳뿐이면 순서가 곧 결과다. 그래서 선별 규칙을 따져야 한다',
...
# ── 9. 분석 설계 1 — 지정 감사 ──────────────────────────────
s = slide()
header(s, '분석 설계 ① 지정 감사 (Q1·Q2)', '공표된 규칙을 그대로 재계산해 실제와 대조한다')
for i, (t, b) in enumerate([
...
# ── 11. 분석 설계 3 — 처방 모형 ─────────────────────────────
s = slide()
header(s, '분석 설계 ③ 처방 — 우선순위·유형 배정 모형 (Q4)', '새 기준을 만들지 않는다: 정부 문서의 임계값만 조합한다')
